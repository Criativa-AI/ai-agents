...
RSpec.describe Agents::Runner do
  ...
  context "when simple conversation without tools" do
    ...
    it "creates RubyLLM chat with provider and assume_model_exists from the agent" do
      mock_chat = instance_double(Agents::RuntimeChat)
      mock_response = instance_double(RubyLLM::Message, tool_call?: false, content: "Hello from Azure",
                                                        input_tokens: 10, output_tokens: 5)

      allow(Agents::RuntimeChat).to receive(:new).with(
        model: "deployment-name",
        provider: :azure,
        assume_model_exists: true,
        run_context: kind_of(Agents::RunContext)
      ...
      expect(Agents::RuntimeChat).to have_received(:new).with(
  ...
  context "with custom headers" do
    ...
    it "passes runtime headers to RubyLLM chat" do
      ...
      allow(Agents::RuntimeChat).to receive(:new).and_return(mock_chat)
      allow(mock_chat).to receive(:add_message)
      allow(Agents::Helpers::MessageExtractor).to receive(:extract_messages).and_return([])
      allow(mock_chat).to receive_messages(with_instructions: mock_chat, with_temperature: mock_chat,
      ...
      expect(mock_chat).to have_received(:with_headers).with("X-Test": "value")
    end
    ...
    it "applies agent default headers when runtime headers are absent" do
      ...
      expect(mock_chat).to have_received(:with_headers).with("X-Agent": "agent-value")
    end
    ...
    it "merges headers giving runtime precedence over agent defaults" do
      ...
      expect(mock_chat).to have_received(:with_headers).with(
        "X-Shared": "runtime",
  ...
  context "with prebuilt RubyLLM content in history" do
    ...
    it "round trips extracted typed history without nesting its content" do
      ...
      source_chat = instance_double(Agents::RuntimeChat, messages: [source_message])
      ...
      runner_instance = described_class.new
      restored_chat = instance_double(Agents::RuntimeChat)
      restored_messages = []
  ...
  context "with tool message history" do
    ...
    context "with a restoration spy" do
      let(:restored_messages) { [] }
      let(:mock_chat) { instance_double(Agents::RuntimeChat) }
  ...
  context "when handoff occurs" do
    ...
    it "returns error when handoff to unregistered agent is attempted" do
      ...
      stub_chat_sequence({ tool_calls: [{ name: "handoff_to_handoffagent", arguments: "{}" }] })
  ...
  context "when standard error occurs" do
    it "handles errors gracefully and returns error result" do
      ...
      allow(Agents::RuntimeChat).to receive(:new).and_raise(StandardError, "Test error")
  ...
  context "when using response_schema" do
    ...
    it "includes response_schema in API request" do
      ...
      expect(schema_request).to have_been_requested.once
    end
